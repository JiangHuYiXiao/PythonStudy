...
# 表现形式：a = 'jianghu'
# 编码方式：unicode
print(a.encode('utf-8'))  #b'jianghu'    #encode是将数据类型：字符串a，转换成数据类型bytes来表示a，然后以utf-8进行编码
...
# 编码方式：utf-8或者GBK,gb2312，ascii
b = a1.encode('utf-8')   #eencode是将数据类型：字符串a，转换成数据类型bytes来表示a，然后以utf-8进行编码
print(b)   # b'\xe6\xb1\x9f\xe6\xb9\x96'

# 为啥我们不用bytes表现字符串，且bytes的编码方式（utf-8,gbk等）都可以直接在硬盘上存储，和进行传输？，
# 那是因为bytes的对于中文的表现形式，为16进制，我们不好识别